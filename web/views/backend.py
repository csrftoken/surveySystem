...
class SurveyReportView(TemplateView):
    ...
    @staticmethod
    def get_survey_data(middle_survey, survey):
        records = models.SurveyRecord.objects.filter(
            **{
                "middle_survey": middle_survey, "survey": survey
            }
        ).select_related()
        suggestions, answers = [], {}
        ...
        for record in records.iterator():
            ...
            if unique_code in answers:
                ...
                answers[unique_code]["score_sum"] = answers[unique_code]["score_sum"] + record.score
            else:
                answers[unique_code] = {
                    "questions": [
                        {
        ...
        choices = {
            ...
            for question in survey.questions.filter(answer_type="single").iterator()
        }
    ...
    def get_context_data(self, **kwargs):
        ...
        instance = models.MiddleSurvey.objects.filter(pk=pk).first()
    ...
    def post(self, request, **kwargs):
        ...
        queryset = self.queryset.filter(
            middle_survey_id=kwargs.get("pk"), survey_id=request.POST.get("survey_id", "")[-1]
        )
...
class SurveyDownloadView(View):
    ...
    def get(self, *args, **kwargs):
        _ = args[0]
        queryset = self.queryset.filter(middle_survey_id=kwargs.get("pk"))
